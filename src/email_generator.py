...
class EmailGenerator:
    ...
    def generate_weekly_email(self, completed_tasks_by_project: Dict[str, List[TodoItem]], week_start: datetime, week_end: datetime) -> Dict[str, str]:
        ...
        if template:
            # Use template
            email_body = self._fill_template(template, completed_tasks_by_project, week_start, week_end)
        else:
            # Generate default email
            email_body = self._generate_default_email(completed_tasks_by_project, week_start, week_end)
        ...
        return {
            'subject': subject,
            'body': email_body,
            'to': self.config.boss_email,
            'from': self.config.your_name
        }
    
    def save_email_draft(self, email_content: Dict[str, str], output_path: str = None) -> str:
    ...
    def _fill_template(self, template: str, completed_tasks_by_project: Dict[str, List[TodoItem]], week_start: datetime, week_end: datetime) -> str:
        ...
        try:
            ...
            print(f"Warning: Template variable {e} not found, using default email format")
            return self._generate_default_email(completed_tasks_by_project, week_start, week_end)
    
    def _generate_default_email(self, completed_tasks_by_project: Dict[str, List[TodoItem]], week_start: datetime, week_end: datetime) -> str:
